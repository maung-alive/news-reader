...
const Item = ({data}) => {
  ...
  return (
    <div className="w-full h-full rounded-2xl overflow-hidden relative bg-gray-400">
      ...
      <div className="absolute bottom-0 left-0 p-2 pb-10 w-full bg-[rgba(255,255,255,0.4)] rounded-t-xl">
        <p className="text-3xl font-medium hover:text-white">{data?.title}</p>
...
export default function Slider({data, className}) {
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className='absolute top-1/2 left-0 -translate-y-1/2 rounded-e-xl text-white hover:text-black duration-200 hover:bg-[rgba(200,200,200,0.4)] p-2' onClick={prevSlide}>
        <FaChevronLeft className='text-3xl' />
      </div>
      <div className='absolute top-1/2 right-0 -translate-y-1/2 rounded-s-xl text-white hover:text-black duration-200 hover:bg-[rgba(200,200,200,0.4)] p-2' onClick={nextSlide}>
